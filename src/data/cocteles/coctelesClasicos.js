const coctelesClasicos = [
    {
        nombre: "Alexander",
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/alexander.jpg",
        ingredientes: ["1 oz Cogñac", "1 oz Licor de Cacao", "1 oz Half and Half"], 
        receta: "En una coctelera con hielo, agrega el coñac, el licor de cacao y el Half and Half.\n Agita vigorosamente durante 15-20 segundos para enfriar y mezclar bien los ingredientes.\n Cuela la mezcla en una Copa Martini o Copa Coupé, previamente enfriada.\n Decorar con una ramita de canela y espolvorear un poco de cacao en polvo por encima.\n ¡Disfruta!",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Americano", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/americano.jpg", 
        ingredientes: ["30 ml de Campari", "30 ml de Vermouth Rosso", "Chorrito de Soda"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Barracuda", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/barracuda.jpg", 
        ingredientes: ["45 ml de Ron Blanco", "15 ml de Licor de Banana", "30 ml de Zumo de Ananá", "60 ml de Champagne"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association" 
    },
    { 
        nombre: "Bee's Knees", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/beesknees.jpg", 
        ingredientes: ["50 ml de Gin", "25 ml de Zumo de Limón", "25 ml de Jugo de Naranja", "2 Cucharaditas de Miel"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Between the Sheets", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/betweenthesheets.jpg", 
        ingredientes: ["30 ml de Ron", "30 ml de Cognac", "30 ml de Triple Sec", "30 ml de Zumo de Limón"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Black Russian", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/blackrussian.jpg", 
        ingredientes: ["45 ml de Vodka", "45 ml de Licor de Café", "30 ml de Café (opcional)"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Bloody Mary", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/bloodymary.jpg", 
        ingredientes: ["45 ml de Vodka", "90 ml de Jugo de Tomate", "15 ml de Zumo Limón", "1 Pizca de Sal Fina", "1 Pizca de Pimienta Negra", "1-3 Gotas de Salsa Picante (como Tabasco)"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Boulevardier", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/boulevardier.jpg", 
        ingredientes: ["45 ml de Whisky", "30 ml de Campari", "30 ml de Vermouth rosso"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Caipirinha", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/caipirinha.jpg", 
        ingredientes: ["75 ml de Cachaca", "30 ml de Almibar", "Lima"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Cosmopolitan", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/cosmopolitan.jpg", 
        ingredientes: ["45 ml de Vodka", "15 ml de Triple Sec", "30 ml de Jugo de Arándano rojo", "15 ml Zumo de Limón"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Cuba Libre", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/cubalibre.jpg", 
        ingredientes: ["45 ml de Ron Blanco", "120 ml de Coca-Cola", "10 ml de Zumo de Limón"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Daiquiri Clásico", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/daiquiriclasico.jpg", 
        ingredientes: ["60 ml de Ron Blanco", "30 ml de Zumo de Limón", "20 ml de Almíbar"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association" 
    },
    { 
        nombre: "Dry Martini", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/drymartini.jpg", 
        ingredientes: ["75 ml de Gin", "15 ml de Vermouth Dry", "Dos o tres Aceitunas"], 
        receta: "Ingredientes y pasos para el Martini...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Espresso Martini", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/espressomartini.jpg", 
        ingredientes: ["45 ml de Vodka", "30 ml de Licor de Café", "30 ml de Café Espresso", "15 ml de Almíbar"], 
        receta: "Ingredientes y pasos para el Martini...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Gancia Batido", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/ganciabatido.jpg",
        ingredientes: ["75 ml de Gancia", "30 ml de Almibar", "Limón"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta establecida por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Grasshopper", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/grasshopper.jpg", 
        ingredientes: ["30 ml de Vodka", "30 ml de Licor de Chocolate Blanco", "15 ml de Licor de Menta", "15 ml de Half and Half"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Hanky Panky", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/hankypanky.jpg", 
        ingredientes: ["45 ml de London Dry Gin", "45 ml de Vermouth Rosso", "10 ml Fernet Branca"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Long Island Iced Tea", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/longislandicedtea.jpg", 
        ingredientes: ["15 ml de Vodka", "15 ml de Ron", "15 ml de Gin", "15 ml de Triple Sec, 30 ml de Sweet & Sour", "Coca-Cola"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Mai-Tai Clásico", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/maitaiclasico.jpg", 
        ingredientes: ["45 ml de Ron Blanco", "15 ml de RonOscuro", "15 ml de Amaretto", "15 ml de Triple Sec", "10 ml de Almíbar", "30 ml de Zumo de Limón"], 
        receta: "Ingredientes y pasos para el Martini...",
        referencia: "Basada en la receta del International Bartenders Association"
    },
    { 
        nombre: "Manhattan", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/manhattan.jpg", 
        ingredientes: ["60 ml de Whisky canadian", "30 ml de Vermouth Rosso", "Gotas de Bitter Angostura"], 
        receta: "Ingredientes y pasos para el Martini...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Margarita", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/margarita.jpg", 
        ingredientes: ["60 ml de Tequila", "45 ml de Triple Sec", "45 ml de Sour Mix"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Martinez", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/martinez.jpg", 
        ingredientes: ["45 ml de London Dry Gin", "45 ml de Vermouth Rosso", "5 ml de Marrasquino", "3 gotas de Bitter Angostura"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Associations"
    },
    { 
        nombre: "Mary Pickford", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/marypickford.jpg", 
        ingredientes: ["45 ml de Ron Blanco", "45 ml de Zumo de Piña", "10 ml de Marrasquino", "5 ml de Granadina"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Mimosa Clásico", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/mimosaclasico.jpg", 
        ingredientes: ["75 ml de Champagne", "75 ml de Zumo de Naranja"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Mint Julep", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/mintjulep.jpg", 
        ingredientes: ["60 ml de Bourbon", "8-10 Hojas de Menta frezca", "10 ml de Almíbar"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Moscow Mule", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/moscowmule.jpg", 
        ingredientes: ["60 ml de Vodka", "120 ml de Cerveza de Jengibre", "15 ml de Zumo de Lima"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Mojito", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/mojito.jpg", 
        ingredientes: ["60 ml de Ron", "30 ml de Sour Mix", "6-8 hojas de Menta", "Soda"], 
        receta: "Ingredientes y pasos para el Martini...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Negroni", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/negroni.jpg", 
        ingredientes: ["30 ml de Gin", "30 ml de Vermouth Rosso", "30 ml de Campari"], 
        receta: "Ingredientes y pasos para el Martini...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "New York Sour", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/newyorksour.jpg", 
        ingredientes: ["60 ml de Bourbon o Whisky", "30 ml de Zumo de Limón", "20 ml de Almibar", "Clara de Huevo", "15 ml de Vino Tinto (como Merlot o Cabernet Sauvignon)"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Old Cuban", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/oldcuban.jpg", 
        ingredientes: ["45 ml de Ron Añejo", "6-8 Hojas de Menta", "30 ml de Zumo de Limón", "20 ml de Almibar", "2 Gotas de Bitter Angostura", "60 ml de Champagne"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Old Fashioned", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/oldfashioned.jpg", 
        ingredientes: ["105 ml de Bourbon", "Una cucharada de Azúcar", "Gotas de Bitter Angostura"], 
        receta: "Ingredientes y pasos para el Martini...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Penicillin", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/penicillin.jpg", 
        ingredientes: ["60 ml de Whisky scoth", "20 ml de Syrup de Gengibre", "20 ml de Syrup de Miel", "30 ml de Zumo de limón"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "Sex on the Beach", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/sexonthebeach.jpg", 
        ingredientes: ["45 ml de Vodka", "30 ml de Licor de Durazno", "75 ml de Jugo de Naranja", "10 ml de Granadina"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Tequila Sunrise", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/tequilasunrise.jpg", 
        ingredientes: ["45 ml de Tequila", "90 ml de Jugo de Naranja", "10 ml de Granadina"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Whiskey Sour", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/whiskeysour.jpg", 
        ingredientes: ["45 ml de Whisky", "30 ml de Zumo de Lima", "20 ml de Almibar Simple", "Clara de Huevo"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "White Lady", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/whitelady.jpg", 
        ingredientes: ["40 ml de Gin", "30 ml de Triple Sec", "20 ml de Zumo de Limón"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta oficial del International Bartenders Association"
    },
    { 
        nombre: "White Russian", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/whiterussian.jpg", 
        ingredientes: ["30 ml de Vodka", "45 ml de Licor de Café", "30 ml de Half and Half", "30 ml de Café (opcional)"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Receta otorgada por el Centro Internacional de Coctelería"
    },
    { 
        nombre: "Zombie Clásico", 
        imagen: "../src/data/cocteles/imagesCocktail/clasicos/zombieclasico.jpg", 
        ingredientes: ["45 ml de Ron Oscuro", "45 ml de Ron Dorado", "20 ml de Zumo de Limón", "15 ml de Falernum", "10 ml de Granadina", "2 gotas de Bitter Angostura"], 
        receta: "Ingredientes y pasos para la Margarita...",
        referencia: "Basada en la receta del International Bartenders Association"
    },
    // { nombre: "", imagen: "../src/data/cocteles/imagesCocktail/clasicos/.jpg", ingredientes: [], receta: "Ingredientes y pasos para la Margarita..." },
];

window.coctelesClasicos = coctelesClasicos;